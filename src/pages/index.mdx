...
## Robustness Results
Our practical method may not converge, but using the tools from our theoretical insights, we get empirical gains. In our method, the adversary's objective is the expected _score_ of the policy over the levels, generally meaning the policy's room for improvement on each level. 
