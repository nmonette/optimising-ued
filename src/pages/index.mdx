...
and the _adversary_ that proposes training levels. Our method formally examines this concept, and alters the optimisation 
...
## Motivation
Recent UED research has focused on optimizing performance on popular benchmarks, but had forgone theoretical clarity as a result. While many of the 
...
## Robustness Results
In our method, the adversary's objective is the expected _score_ of the policy over the levels, namely meaning the policy's room for improvement on each level. 
